...
//		//int[][][] arr31=new int[3][][]; 
		...
		case 3:
			...
			break;
		case 4:
			int arrmax = Array2DService.maxOfAllNumbers(arr);
			System.out.println("Maximum number: " + arrmax);
			break;
		case 5:
		    int[] rowMax = Array2DService.maxRowwise(arr);
		    for (int i = 0; i < rowMax.length; i++) {
		        System.out.println("Max of row " + i + ": " + rowMax[i]);
		    }
		    break;

		case 6:
			int [] colMax = Array2DService.maxColumnwise(arr);
			  for (int i = 0; i < colMax.length; i++) {
			        System.out.println("Max of column " + i + ": " + colMax[i]);
			    }
		    break;
		case 7:
