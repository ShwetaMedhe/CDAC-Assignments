...
public class ProductDaoImpl implements ProductDao {
	...
	static PreparedStatement selproduct,insproduct,deletById,selById,updateById;
	...
	static {
		...
		try {
			...
			insproduct=conn.prepareStatement("insert into myproduct1 values (?,?,?,?,?,?)");
			deletById=conn.prepareStatement("delete from myproduct1 where pid=?");
			selById=conn.prepareStatement("select * from myproduct1 where pid=?");
			updateById=conn.prepareStatement("update myproduct1 set pname=?,qty=?,price=?,expdate=?,cid=? where pid=?");
	...
	public boolean removeById(int pid) {
		try {
			deletById.setInt(1, pid);
			int n=deletById.executeUpdate();
			return n>0;
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
