...
public class ProductDaoImpl implements ProductDao{
	...
	public List<Integer> findAllCategory() {
		try {
			PreparedStatement getcat=conn.prepareStatement("select distinct cid from myproduct1 ");
	...
	public List<Product> findByCategory(int cid) {
		try {
			PreparedStatement getprod=conn.prepareStatement("select * from myproduct1 where cid=? ");
